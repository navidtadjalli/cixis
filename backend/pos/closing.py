...
def settled_order_receipts(orders):
    """Serialize settled orders with their full ticket and payment detail."""
    return [
        {
            "id": o.id,
        ...
        for o in orders.select_related("table").prefetch_related("items", "payments")
    ]
...
def compute_day_summary(business_date: date | None) -> dict:
    ...
    settled = settled_order_receipts(closed_orders)
