...
def close(request):
    ...
    if DayClosing.objects.filter(business_date=today).exists():
        return Response(
            {"detail": "روز جاری قبلاً بسته شده است."},
            status=status.HTTP_400_BAD_REQUEST,
        )

    summary = closing.compute_day_summary(today)
    ...
    day_closing = closing.create_day_closing(today)
